...
const updateGame = asyncHandler(async function(req,res){
    ...
    const{name,fromTime,updatedFromTime,updatedToTime,price,location} = req.body;
    const user = req.user;
    ...
    const isTimeSlotAvailable = await timeSlotAvailable(updatedFromTime,updatedToTime,name)
    if(isTimeSlotAvailable === true){

        const game = await Game.updateOne({fromTime:new Date(fromTime),name},{
            "user" : req.user.id,
            name,
            location,
